...
import {
  usePermissionTypeFindById,
  usePermissionTypeFindInfinite,
} from '@modules/permission-types/lib/hooks';
...
import { usePermissionFind } from '../lib/hooks';
...
const PermissionsForm: React.FC<IProps> = ({
  ...
  const permissionTypeQuery = usePermissionTypeFindById({
    id: initialValues?.permissionType,
    config: {
      queryKey: [],
      enabled: !!initialValues?.permissionType,
    },
  });

  const permissionTypesQuery = usePermissionTypeFindInfinite({
    options: {
      limit: 20,
                ...
                .filter(
                  (permission) =>
                    !permissionsQuery.data?.data
                      ...
                      .includes(permission) || initialValues?.title === permission,
                )
                .map((permission) => ({
                  key: permission,
                  label: permission,
