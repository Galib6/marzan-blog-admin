...
const useTheme = () => {
  ...
  const [theme, setTheme] = useIndexedDBState<TThemeType>(States.theme);
  const [systemTheme, setSystemTheme] = useState<Exclude<TThemeType, ENUM_THEME_TYPES.SYSTEM>>(
    ENUM_THEME_TYPES.LIGHT,
  );
  ...
  const handleToggleThemeFn = () => {
    const nextTheme =
      resolvedTheme === ENUM_THEME_TYPES.DARK ? ENUM_THEME_TYPES.LIGHT : ENUM_THEME_TYPES.DARK;
    setTheme(nextTheme);
  };

  const handleApplyThemeFn = (
    theme: TThemeType,
    systemThemeValue?: Exclude<TThemeType, ENUM_THEME_TYPES.SYSTEM>,
  ) => {
    if (!isClient) return;
  ...
  return {
    isLight: isClient ? resolvedTheme !== ENUM_THEME_TYPES.DARK : true, // Default to light theme during SSR
    isDark: isClient ? resolvedTheme === ENUM_THEME_TYPES.DARK : false, // Default to false during SSR
    theme,
    setTheme,
    handleToggleThemeFn,
  };
